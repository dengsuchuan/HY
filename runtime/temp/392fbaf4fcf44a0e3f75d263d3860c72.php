<?php /*a:2:{s:92:"I:\Project\WebServer\www\project\Hy\application\index\view\production_records\edit_view.html";i:1542081053;s:77:"I:\Project\WebServer\www\project\Hy\application\index\view\public\header.html";i:1541478155;}*/ ?>
 <!doctype html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<div class="x-body">
    <form class="layui-form">
        <?php if(is_array($productLog) || $productLog instanceof \think\Collection || $productLog instanceof \think\Paginator): $i = 0; $__LIST__ = $productLog;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$logList): $mod = ($i % 2 );++$i;?>
        <div class="layui-form-item">
            <label class="layui-form-label">记录编号</label>
            <div class="layui-input-block" >
                <input type="text" class="layui-input" name="log_id" value="<?php echo htmlentities($logList['log_id']); ?>" disabled >
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">任务编号</label>
            <div class="layui-input-block" >
                <input type="text" class="layui-input" name="task_id" value="<?php echo htmlentities($logList['task_id']); ?>" disabled>
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">员工</label>
            <div class="layui-input-block" >
                <input type="text" class="layui-input" name="hr_id" value="<?php echo htmlentities($logList['hr_id']); ?>" disabled >
            </div>
        </div>
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <select name="complete_date" lay-verify="required">
                    <option value='<?php echo htmlentities($logList['complete_date']); ?>'><?php echo htmlentities($logList['complete_date']); ?></option>
                    <optgroup label="选择日期">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline" >
                <input type="text" class="layui-input" name="process_id" id="process_id" value="<?php echo htmlentities($logList['process_id']); ?>" disabled>
            </div><a class="layui-btn layui-btn-sm layui-btn-normal" id="process">选择工艺</a>
        </div>
        <div class="layui-form-item" hidden id="processInfo">
            <div class="layui-input-block" >
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block" >
                <input type="number" class="layui-input" name="complete_qty" id="complete" value="<?php echo htmlentities($logList['complete_qty']); ?>" lay-verify="required" title="选择工艺后才可修改" disabled>
                <br>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <select name="eqpmt_id" lay-verify="required" lay-search="" >
                    <option value="<?php echo htmlentities($logList['eqpmt_id']); ?>"><?php echo htmlentities($logList['eqpmt_id']); ?></option>
                    <optgroup label="选择设备">
        ...
        <input type="hidden" name="update_name" value="<?php echo htmlentities(app('session')->get('user.user_name')); ?>">
        <div class="layui-form-item">
            <label class="layui-form-label">合格品数量</label>
            <div class="layui-input-block" >
                <input type="number" class="layui-input" name="product_qty" id="product_qty" value="<?php echo htmlentities($logList['product_qty']); ?>" lay-verify="required" placeholder="禁止修改:请填写完成数量参数" disabled>
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">质量等级</label>
            <div class="layui-input-block" >
                <input type="text" class="layui-input" name="quality_grade" value="<?php echo htmlentities($logList['quality_grade']); ?>" id="quality_grade" lay-verify="required">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">是否审核</label>
            <div class="layui-input-block">
                <input type="checkbox" name="if_audit" lay-skin="switch" lay-text="已审|未审" <?php echo $logList['if_audit']=="未审" ? "" : "checked=''"; ?>>
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">是否外协</label>
            <div class="layui-input-block">
                <input type="checkbox" name="fulfill" lay-filter="fulfill" lay-skin="switch" lay-text="是|否" <?php echo $logList['fulfill']==0 ? "" : "checked=''"; ?>>
            </div>
            <p style="color: red" id="costP" hidden>忽略显示，外协单位和外协费用已清空，请重新选择。</p>
        </div>
        <div id="cost" <?php echo $logList['fulfill']=="1" ? "" : "hidden"; ?> >
            <div class="layui-form-item">
                <label class="layui-form-label">外协费用</label>
                <div class="layui-input-block">
                    <input type="text" class="layui-input" id="costInput" name="cost" value="<?php echo htmlentities($logList['cost']); ?>">
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-block">
                    <select name="costUnit" lay-search="" >
                        <option value="<?php echo htmlentities($logList['costUnit']); ?>"><?php echo htmlentities($logList['costUnit']); ?></option>
                        <optgroup label="选择客户">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block" >
                <input type="text" class="layui-input" name="remark" value="<?php echo htmlentities($logList['remark']); ?>">
            </div>
        </div>
        <?php endforeach; endif; else: echo "" ;endif; ?>
        <div class="layui-form-item">
...
<script>
    ...
    layui.use(['form'], function(){
        ...
        form.on('submit(addId)', function(data){
            ...
            $.post("<?php echo url('index/ProductionRecords/saveEdit'); ?>",data.field,function(info){
                if (info) {
